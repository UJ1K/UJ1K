# Hi, there! I'm Fauzi 👋

<p align="center">
  <img src="https://your-custom-banner-url.com/banner.png" alt="Building the Future of Remote Sensing" width="100%">
</p>

### 🛰️ Geospatial Analyst & Remote Sensing Researcher
Current Focus: **Universitas Gadjah Mada** | Satellite Imagery & Python Automation

---

## 🚀 About Me
I specialize in solving complex environmental challenges through the lens of spatial data. From atmospheric remote sensing to GIS modeling, I build automated workflows to extract insights from the earth's surface.

* 🔭 **Currently working on:** Satellite imagery processing at UGM.
* 🌱 **Learning:** Advanced GIS Backend services & High-performance computing.
* 💬 **Ask me about:** Google Earth Engine, Python for GIS, and Climate Management.

---

## 🛠️ Tech Stack

| Category | Tools & Technologies |
| :--- | :--- |
| **Cloud Computing** | ![GEE](https://img.shields.io/badge/Google_Earth_Engine-4285F4?style=flat-square&logo=google&logoColor=white) |
| **Programming** | ![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white) ![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black) ![SQLite](https://img.shields.io/badge/SQLite-003B57?style=flat-square&logo=sqlite&logoColor=white) |
| **Analysis** | ![GDAL](https://img.shields.io/badge/GDAL-006633?style=flat-square) ![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat-square&logo=pandas&logoColor=white) |
| **GIS Software** | ![ArcGIS](https://img.shields.io/badge/ArcGIS-005E95?style=flat-square) ![QGIS](https://img.shields.io/badge/QGIS-589632?style=flat-square&logo=qgis&logoColor=white) |

---

## 📂 Featured Projects
* **[Map-Navigation-Tools](https://github.com/UJ1K/Map-Navigation-Tools)**: Interactive Web GIS for Network Analysis (React, OpenLayers).
* **[Himawari](https://github.com/UJ1K/Himawari)**: Cloud segmentation using Python based on Himawari-8/9 data.
* **[Sentiment Analysis](https://github.com/UJ1K/Sentiment_Analysis_For_Business)**: Data-driven decision making using NLP.

---

## 📬 Reach me on:
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/fauzi-s-367150258/)
[![Kaggle](https://img.shields.io/badge/Kaggle-20BEFF?style=for-the-badge&logo=Kaggle&logoColor=white)](https://www.kaggle.com/defaultuj1k)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:fauzisalam45@gmail.com)
